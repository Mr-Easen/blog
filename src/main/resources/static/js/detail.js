    ...
    Comment: function(c,index){
        ...
        var text = '<div class="commentList-box cbox-'+c.id+'" id="'+c.id+'">'
            ...
            text += '<div class="fl"><p class="name"><a href="'+blog_url+c.senderId+'">'+c.senderName+'</a>'
                if(c_user_id == c.senderId){
                    text += '<span><i></i>作者</span>'
                }
            text += '</p>'
            text += '<p class="time">'+c.floor+'楼&nbsp;&nbsp;'+_this.Time(c.createTime)+'</p></div><div class="clear"></div></div>'                        
...
//            text += '<p class="zan" blog_id="'+c.id+'" type="2" userid="'+c.user_id+'">'+(c.up_num ? c.up_num : 0)+'</p>' //人赞
                if(is_comment != 2) {
                    ...
                    text += '<p class="answer reply-btn"><span class="comment_number_1">'+(c.reply == null ? 0 : c.reply.length)+'</span>人回复</p>'
                }
            console.info("i_user_id:"+i_user_id);
                if(i_user_id == c.senderId) {
                    text += '<p class="remove">删除</p>'
                }
            text += '<div class="clear"></div></div>'
            text += '<input type="hidden" class="reply_id" value="'+c.id+'"><input type="hidden" class="user_id" value="'+c.senderId+'"><input type="hidden" class="first_pid" value="'+c.senderId+'"></div>'
            if(c.reply) {//二级回复
                ...
                text += '<input type="hidden" class="floor" value="'+c.reply.floor+'"><div class="top">'
                if(c.reply.length > 0){
    ...
    CommentChild: function(i,r){
        ...
        text += '<input type="hidden" class="floor" value="'+r.floor+'"><div class="top">'
        if(c_user_id == r.senderId) {
            text += '<a class="author fl">作者</a>'
        }
        text += '<a class="name fl" href="'+blog_url+r.senderId+'">'+r.senderName+'</a><a class="name fl" href="'+blog_url+r.receiverId+'">:@'+r.receiverName+'&nbsp;</a>'
        text += r.content+'<div class="clear"></div></div>'
        text += '<div class="bot"><p class="time">'+_this.Time(r.createTime)+'</p>'
        if(is_comment != 2) {
            if(i_user_id != r.user_id)
            text += '<p class="reply reply-btn">回复</p>'
        }
        if(i_user_id == r.senderId) {
            text += '<p class="remove">删除</p>'
        }
        text += '<div class="clear"></div></div>'
        text += '<input type="hidden" class="reply_id" value="'+r.id+'"><input type="hidden" class="user_id" value="'+r.senderId+'"><input type="hidden" class="first_pid" value="'+r.first_pid+'">'
        text += '</li>'
        return text;
    },
    Focus: function(){
    ...
    SetReplay: function(){//发表评论参数
        ...
        $('.publish-btn').click(function(){
            ...
            var first_pid = ''
        	var floor = me.closest('.publish_user_id').find('.floor').val()
            var isReply = parent_id == '' ? 0 : 1;
            if(flag==2){
    ...
    SetReplayDown: function(ev){//ctrl+enter发表评论
        ...
        $('.textareadiv').keydown(function(ev){
            if(ev.ctrlKey && ev.keyCode == 13){
                ...
                var me = $(this)
                var content = me.val()
                var user_id = me.closest('.publish_user_id').find('.user_id').val()
...
//        var page = $("#sort-"+comment_sort).attr("page");
        ...
        page = parseInt(page);
        pagesize = 10;
        var data = {articleId: blog_id,sort: comment_sort,page: page}
        $.get(comment_list,data,function(e){
            _this.is_load = false;
            if(e.status == 0){
            	$(".comment_number").text(e.count);
            	$("#commentCount").text('评论('+e.count+')');
//                _this.SetPage(e.data.current_page, e.data.page_count);
                ...
                for (var i = 0; i < e.list.length; i++) {
                    ...
                    content_text += _this.Comment(e.list[i], index);
                }
...
//                setPraise($('.comment-1 .bot .zan'))//点赞
    ...
    SetAddReply: function(e,data){//添加评论和回复
        var _this = this
        var par = '';
        if(e.data.isReply == 1){//回复
            if($('.cbox-'+data.parentId).hasClass('comment-2-cb')){
                par = $('.cbox-'+data.parentId).parent()
                par.html(_this.CommentChild(1,e.data) + par.html())
                _this.ScrollTopFn($(par))
            }else{
                var newReply = '<div class="new-comment"><input type="hidden" class="floor" value="'+e.data.floor+'"></div>'
                par = $('.cbox-'+data.parentId).find($('.comment-2-list'))
    ...
    Remove: function(){//删除回复
        ...
        $('.remove').click(function(){
            ...
            new AutoBox({content:'<div style="padding: 0 30px 30px 30px;">是否要删除发布的评论？</div>',img:'remind',mask:"#000",Yes:'确定',No:'取消',
                yc:function(){
                    $.get(removeUrl,{id: id},function(e){
                        if(e.status == 1){
                            if(e.data) {
                                for (var i = 0; i < e.data.length; i++) {
                                    var class_name = "cbox-" + e.data[i].id;
                                    $("." + class_name).remove();
                                }
                            }
